...
def weight_cross_entropy(pred, gt, wt=False, weight=None):
    global weight_table
    batch = gt.shape[0]
    ...
    for i in range(len(gt)):
        if wt:
            loss -= weight[i] * torch.log(pred_exp_weight[i][gt[i]] / torch.sum(pred_exp_weight, 1)[i])
        else:
