...
from utils.ssim_psnr import SSIM
class ImageLoss(nn.Module):
    # 1e-4
    def __init__(self, gradient=True, loss_weight=[20, 1e-2]):
        ...
        self.ssim = SSIM()
    def forward(self, out_images, target_images, grad_mask=None, label_strs=None):
        mse_loss = self.mse(out_images, target_images).mean(1).mean(1).mean(1)
        # print(mse_loss.mean()*self.loss_weight[0], self.GPLoss(out_images[:, :3, :, :], target_images[:, :3, :, :])*self.loss_weight[1])
        # ssim_loss = self.ssim(out_images, target_images)
