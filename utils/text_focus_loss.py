...
from utils.weight_ce_loss import weight_cross_entropy
...
class TextFocusLoss(nn.Module):
    def __init__(self, args):
        ...
        print("Build Text Focus Loss")

    def build_up_transformer(self):

        transformer = Transformer().cuda()
        transformer = nn.DataParallel(transformer)
        transformer.load_state_dict(torch.load('./pths/pretrain_transformer.pth'))
        transformer.eval()
    ...
    def forward(self,sr_img, hr_img, label_strs):
        label = label_strs
        ...
        if True:
            ...
            # print(sr_img.size(),'vs',hr_img.size(),'vs',text_gt.size(),sr_pred.size())
            # os._exit(233) 
