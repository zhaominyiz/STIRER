...
class RecurrentResidualBlockTL(nn.Module):
    def __init__(self, channels=60, text_channels=288, clue=False):
...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False,
                 img_size=224, patch_size=4, resi_connection='1conv',is_eval=False):
        ...
        self.patch_unembed = PatchUnEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=0, embed_dim=dim,
            norm_layer=None)
        self.gru = RecurrentResidualBlockTL(dim,288)
        self.is_eval = is_eval
        if not is_eval:
            self.vit = ViTEncoder(img_size=(16,64),patch_size=(16,2),in_chans=48)
            self.fc = nn.Linear(192,CHARSET)

            self.conv_after_body = nn.Conv2d(dim, dim, 3, 1, 1)
            patches_resolution = self.patch_embed.patches_resolution
            self.upsample = UpsampleOneStep(2, dim, 4,
        ...
        #     num_labels=charset
        #     )
        # self.rec_block = BertModel(encoder_config)

    def forward(self, x, x_size, clue=None):
        y = self.conv(self.patch_unembed(self.residual_group(x, x_size), x_size))
        # print(y.shape,x.shape)
        # TODO Remove GRU
        y = self.gru(y, clue)
        # remember to move
        # y = self.patch_unembed(x, x_size)
        # y = self.gru(y, clue)
        rec = None
        sr = None
        if not self.is_eval:
            sr = self.conv_after_body(y) + y
            sr = self.upsample(sr)
            # print(y.shape,'233')
            rec = self.vit(y)
            # print(rec.shape)
...
class STIRER(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=4,
                 ...
                 use_checkpoint=False, upscale=2, img_range=1., upsampler='', resi_connection='3conv', 
                 is_eval=False,
                 **kwargs):
        super(STIRER, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
        ...
        self.last_sr = RecurrentResidualBlockTL(channels=48+4)
        self.down_conv = nn.Conv2d(48+4, 48, 3, 1, 1)
        ...
        # Transformer Encoder
        self.vit =  ViTEncoder(img_size=(16,64),patch_size=(16,2),in_chans=48+4,depth=3,embed_dim=96)
        # self.fc = nn.Linear(192,CHARSET)
        # Transformer Decoder
        from transformers import T5Tokenizer, T5Model, T5ForConditionalGeneration, T5Config
        # 0 for pad and start; +1 for EOS
        config = T5Config(
            # another for EOS
            vocab_size = CHARSET+1,
            d_model=96,
            d_ff=96*4,
            num_layers=3,
        ...
        # self.decoder = CRNN(32, 52, 37, 256)
    ...
    def forward_features(self, x, clue=None):
    ...
    def forward(self, x, labels=None):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # ''' AR
            y = self.vit(torch.cat((res[0],orgx),1))
            if self.training:
                # print(x.shape,orgx.shape)
                # print(y.shape,'wow')
                if labels is None:
                    labels = torch.zeros(x.size(0),31).to(x.device).long()
                # print("AA",y.shape,labels.shape)
                y = self.decoder(inputs_embeds = y, labels= labels)
                # print(y.logits.shape)
                # print('loss=',y.loss)
            else:
                y = self.decoder.generate(inputs_embeds = y, num_beams=1)
            # '''
            
            orgx = orgx.detach()
            xx = res[0].detach()
            residual = x.clone().detach()
            xx = self.last_sr(torch.cat((orgx,res[0]),1))
            xx = self.down_conv(xx)
            x = self.conv_after_body(xx) + residual
            x = self.upsample(x)
        elif self.upsampler == 'nearest+conv':
            # for real-world SR
            x = self.conv_first(x)
        ...
        return x[:, :, :H*self.upscale, :W*self.upscale], y, res[1], res[2]
...
class ViTEncoder(VisionTransformer):
    ...
    def __init__(self, img_size=(16,64), patch_size=(16,2), in_chans=4, embed_dim=192, depth=6, num_heads=3, mlp_ratio=4.,
...
if __name__ == '__main__':
    ...
    model = STIRER(upscale=2, img_size=(16, 64),
                   ...
                   embed_dim=48, num_heads=[6, 6, 6], mlp_ratio=2, upsampler='pixelshuffledirect')
    # print(model)
    print(height, width, model.flops() / 1e9)

    x = torch.randn((1, 4, height, width))
    x = model(x)
    sr, logit, srs, logits = x
    print(sr.shape, logit.shape)
    for i in range(len(srs)):
        print('ITER',srs[i].shape,logits[i].shape)